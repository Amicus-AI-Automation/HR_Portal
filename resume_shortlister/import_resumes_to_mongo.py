"""
Bulk import all PDFs in the resumes/ folder into MongoDB.
Run once from the resume_shortlister directory:
    python import_resumes_to_mongo.py

Each document stored in the 'resumes' collection will have:
    - filename   : e.g. "1.pdf"
    - path       : absolute path on disk
    - raw_text   : extracted text (via the existing parser)
    - pdf_content: raw PDF bytes stored as BSON Binary
    - name        : extracted candidate name (or None)
    - email       : extracted email (or None)
    - phone       : extracted phone  (or None)

No 'contact' nested object. No 'created_at' field.
"""
